...
export const deployPoolFactoryContract = async ({
	...
	return hre.deployOrUpgradeProxy<PoolFactory, PoolFactory__factory>({
		contractName: 'PoolFactory',
		// TODO:sw from config by environment
		args: await Promise.all([
			hre.ethers.getNamedSigner('admin').then((s) => s.address),
			hre.ethers.getContract('Pool').then(async (c) => {
...
export const deployPoolContract = async ({
	...
	return hre.deployOrUpgradeProxy<Pool, Pool__factory>({
		contractName: 'Pool',
		args: [
			await hre.ethers.getNamedSigner('admin').then((s) => s.address),
